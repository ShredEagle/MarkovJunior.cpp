...
void ParallelNode::addMatch(const int ruleIndex,
                            const math::Position<3, int> & aMatchPosition,
                            std::vector<bool> &)
{
        ...
        for (int y = 0; y < rule.mOutputSize.height(); y++) {
            for (int x = 0; x < rule.mOutputSize.width(); x++) {
                unsigned char newValue =
                    rule.mOutputs.at(getFlatIndex({x, y, z}, rule.mOutputSize));

                math::Position<3, int> statePos =
                    aMatchPosition + math::Vec<3, int>{x, y, z};
...
bool ParallelNode::run()
{
    ...
    for (unsigned int n = mInterpreter->mFirst.at(mInterpreter->mCounter);
         n < mInterpreter->mChanges.size(); n++) {
