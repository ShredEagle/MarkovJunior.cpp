    ...
    mProbability{aXmlNode.attribute("p").as_double(1.0)}
{
    ...
    if (valueString.compare("") != 0) {
        ...
        for (std::size_t i : splitIntervals(sumsString)) {
            mSums.at(i) = true;
        }
    }
}

ConvolutionNode::ConvolutionNode(const pugi::xml_node & aXmlNode,
                                 const SymmetryGroup &,
    ...
    mSteps{aXmlNode.attribute("steps").as_int(-1)}
{
    ...
    if (mGrid->mSize.depth() == 1) {
        mKernel = std::vector<int>{gTwoDKernels.at(neighborhood).begin(),
                                   gTwoDKernels.at(neighborhood).end()};
    } else {
        mKernel = std::vector<int>{gThreeDKernels.at(neighborhood).begin(),
                                   gThreeDKernels.at(neighborhood).end()};
    }
}
...
bool ConvolutionNode::run()
{
    ...
    mSumField = std::vector<std::vector<int>>(
        mGrid->mState.size(), std::vector<int>(mGrid->mCharacters.size(), 0));
                ...
                for (int offsetZ = -1; offsetZ <= 1; offsetZ++) {
                    for (int offsetY = -1; offsetY <= 1; offsetY++) {
                        for (int offsetX = -1; offsetX <= 1; offsetX++) {
                            math::Position<3, int> offsetPos =
                                math::Position<3, int>{x, y, z}
                                + math::Vec<3, int>{offsetX, offsetY, offsetZ};
                            ...
                            sums.at(grid.mState.at(grid.getFlatGridIndex(offsetPos))) +=
                                mKernel.at(offsetX + 1 + (offsetY + 1) * 3
                                           + (offsetZ + 1) * 9);
                        }
                    }
                }
    ...
    for (std::size_t i = 0; i < mSumField.size(); i++) {
